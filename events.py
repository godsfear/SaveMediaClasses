...
Правила:
  ...
  - Один EventBus на всё приложение, создаётся в Services.create и раздаётся
    через DI-контейнер (svc.bus).
"""
...
class ResumeDownloadEvent:
    """Запрос из истории: возобновить незавершённую (или повторить неудачную)
    загрузку. Обрабатывает DownloadOrchestrator (реконструирует снимок и
    запускает), навигация переключает на главный экран. Несёт всё для
    реконструкции (params — снимок без url) + имя для карточки."""
    task_id: str
    url:     str
    source:  str
    params:  dict
